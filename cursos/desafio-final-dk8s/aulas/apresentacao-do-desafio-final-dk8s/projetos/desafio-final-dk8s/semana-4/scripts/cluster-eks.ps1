...
function Remove-StaleCFStacks {
    ...
    foreach ($stack in $stacks) {
        ...
        if ($status -and $status -ne "DELETE_COMPLETE") {
            Write-Host "  Stack orfa encontrada: $stack ($status) -- deletando..." -ForegroundColor Yellow
            aws cloudformation delete-stack --stack-name $stack --region $REGION
            Write-Host "  Aguardando delecao de $stack..." -ForegroundColor DarkGray
...
function New-EKSCluster {
    ...
    # Limpar stacks orphãs antes de criar (evita AlreadyExistsException)
...
function Get-EKSKubeconfig {
    ...
    if ($Merge) {
        ...
        # Remove lock file stale que eksctl deixa após interrupções
        $lockFile = "$defaultKubeconfig.lock"
        if (Test-Path $lockFile) {
            Remove-Item $lockFile -Force
            Write-Host "  Lock file removido: $lockFile" -ForegroundColor DarkGray
        }
        ...
        if (Test-Path $defaultKubeconfig) {
            ...
            # Remove stale eks-tipsbank entries before merging (só deleta se existir)
