...
if [ "$CALICO_PODS" -gt 0 ]; then
    ...
    # Check for Flannel
    FLANNEL_PODS=$(kubectl get pods -n kube-flannel --no-headers 2>/dev/null | grep -c "Running")
    if [ "$FLANNEL_PODS" -eq "$TOTAL_NODES" ]; then
        check_pass "Flannel is running on all nodes ($FLANNEL_PODS/$TOTAL_NODES)"
    elif [ "$FLANNEL_PODS" -gt 0 ]; then
        check_warn "Flannel is running on $FLANNEL_PODS/$TOTAL_NODES nodes"
    else
        check_fail "No CNI plugin (Calico or Flannel) is running"
    fi
fi
echo ""
